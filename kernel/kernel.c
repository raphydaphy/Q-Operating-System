...
void launchShell() {
    ...
    #define SYSTEMMAN print("\nSystem Utility\nCurrently only able to reboot and halt system.\nHard shutdown to power off.\n\nMenu:\n1. [reboot]\n2. [halt]\n3. [shutdown]\n> ", 0x0F); readStr(bufStr, bufSize); if(strEql(bufStr, "reboot")) { reboot(); } else if(strEql(bufStr, "halt")) { halt(); } else if(strEql(bufStr, "shutdown")) { shutdown(); }
    #define SAYHI print("\nHello!", 0x3F);
    ...
    #define CLEAR clearScreen(); cursorX = 0; cursorY = 0; updateCursor();
    #define BIGCLEAR CLEAR; printIntro();
    ...
    while (true)
    {
        ...
        else if(strEql(bufStr, "clear"))        {   CLEAR;            }
